...
const tables: {
  ...
  indices?: { name: string; columnNames: string[] }[];
}[] = [
  {
    name: 'users',
    columns: [
  ...
  },
  {
    ...
    indices: [{ name: 'idx_kyc_user_id', columnNames: ['user_id'] }],
  },
  {
    name: 'contests',
    columns: [
  ...
  },
  {
    ...
    indices: [
      {
        name: 'idx_notification_logs_user_read',
  ...
  },
  {
    ...
    columns: [
      ...
      { name: 'maxDailySpins', type: 'integer', default: '1' },
      {
        name: 'supportEmail',
        type: 'varchar',
        default: "'support@dreamhome11.com'",
      },
...
export class InitialSchema implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_kyc_aadhaar_unique" ON "kyc" ("aadhaar_number")`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_kyc_pan_unique" ON "kyc" ("pan_number")`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_contests_invite_code_unique" ON "contests" ("invite_code") WHERE invite_code IS NOT NULL`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_achievements_key_unique" ON "achievements" ("key")`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_contest_members_unique" ON "contest_members" ("contest_id", "user_id")`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_likes_unique" ON "likes" ("post_id", "user_id")`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_poll_votes_unique" ON "poll_votes" ("user_id", "poll_id")`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_compensation_logs_unique" ON "compensation_logs" ("contest_id", "user_id")`,
    );
    await queryRunner.query(
