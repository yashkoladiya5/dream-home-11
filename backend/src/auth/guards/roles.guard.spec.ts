...
describe('RolesGuard', () => {
  ...
  describe('when admin role is required', () => {
    it('should allow admin users', () => {
      ...
      const result = guard.canActivate(createMockContext({ role: UserRole.ADMIN }));
      expect(result).toBe(true);
    });

    it('should deny regular users with ForbiddenException', () => {
      mockReflector.getAllAndOverride.mockReturnValue([UserRole.ADMIN]);
      expect(() => guard.canActivate(createMockContext({ role: UserRole.USER }))).toThrow(ForbiddenException);
    });
  ...
  describe('when multiple roles are required', () => {
    it('should allow users with any matching role', () => {
      ...
      expect(guard.canActivate(createMockContext({ role: UserRole.MODERATOR }))).toBe(true);
    });
