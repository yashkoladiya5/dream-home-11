...
export class SmsService {
  ...
  async sendSms(phoneNumber: string, message: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[Mock SMS] Failed to send to ${phoneNumber}: ${(error as Error).message}`);
    }
  }

  async sendCompensationSms(phoneNumber: string, points: number, contestTitle: string): Promise<void> {
