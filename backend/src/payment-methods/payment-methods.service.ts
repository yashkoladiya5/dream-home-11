...
export class PaymentMethodsService {
  ...
  async findByUser(userId: string, category?: string): Promise<SavedPaymentMethod[]> {
  ...
  async getAvailableCategories(): Promise<{
    categories: { key: string; label: string; icon: string; description: string }[];
  }> {
    return {
      categories: [
        { key: 'upi', label: 'UPI', icon: 'phone_android', description: 'Google Pay, PhonePe, Paytm UPI' },
        { key: 'card', label: 'Credit / Debit Card', icon: 'credit_card', description: 'Visa, Mastercard, RuPay' },
        { key: 'net_banking', label: 'Net Banking', icon: 'account_balance', description: 'HDFC, SBI, ICICI & more' },
        { key: 'wallet', label: 'Wallets', icon: 'account_balance_wallet', description: 'Paytm, Freecharge, Mobikwik' },
      ],
    };
  }
}
