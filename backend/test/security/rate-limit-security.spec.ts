...
function mockRequestContext(overrides: Record<string, any> = {}): ExecutionContext {
  const req: any = {
    ip: '192.168.1.1',
    ips: [],
...
describe('Security: Rate Limiting', () => {
  ...
  beforeAll(async () => {
    ...
    throttlerGuard = module.get<ThrottlerBehindProxyGuard>(ThrottlerBehindProxyGuard);
  ...
  describe('User-Based Rate Limiting', () => {
    ...
    function setupUserRateLimitGroup(group: string, limit: number) {
      jest.spyOn(Reflector.prototype, 'getAllAndOverride').mockReturnValue({ group, limit, ttl: 60000 });
    }
    ...
    test('rate limit exceeded returns 429', async () => {
      ...
      (userRateLimitGuard as any).redis = redisMock;
      const ctx = mockRequestContext({ user: mockUser });
      await expect(userRateLimitGuard.canActivate(ctx)).rejects.toThrow(HttpException);
    });
    ...
    test('rate limit headers are set when under limit', async () => {
      ...
      const ctx = mockRequestContext({ user: mockUser });
      ctx.switchToHttp = () => ({
        getRequest: () => ({ ip: '192.168.1.1', url: '/api/v1/test', method: 'GET', user: mockUser, headers: {} }),
  ...
  describe('Rate Limit Bypass Prevention', () => {
    test('health and metrics endpoints bypass rate limiting', async () => {
      jest.spyOn(Reflector.prototype, 'getAllAndOverride').mockReturnValue(null);
  ...
  describe('Rate Limit Violation Logging', () => {
    test('rate limit violations are logged for authenticated users', async () => {
      ...
      jest.spyOn(Reflector.prototype, 'getAllAndOverride').mockReturnValue({ group: 'auth', limit: 5, ttl: 60000 });
      const auditLogSpy = jest.spyOn(auditLogService, 'log').mockResolvedValue({} as any);
      const redisMock = {
        incr: jest.fn().mockResolvedValue(6),
        pexpire: jest.fn().mockResolvedValue(1),
        pttl: jest.fn().mockResolvedValue(30000),
      ...
      (userRateLimitGuard as any).redis = redisMock;
      const ctx = mockRequestContext({ user: mockUser });
      try { await userRateLimitGuard.canActivate(ctx); } catch {}
